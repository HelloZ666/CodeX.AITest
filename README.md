...
- AI 辅助工具：AI 助手问答，支持附件分析、多轮对话与上下文续聊；文档核对支持按项目模板上传、待核对 PDF 上传、源文件样式预览、文字级差异高亮、OCR 文本修正重比对和人工改判记录；回归验证支持数据库字段非空与枚举次数扫描；端到端测试支持多系统数据库字段一致性验证
...
| 功能测试 | 案例生成 | - | 路由 `/functional-testing/case-generation`；支持选择项目、配置是否复用系统功能全景图大纲模板、选择提示词与 AI 推理强度，其中推理强度入口位于工作台头部右侧的 AI 生成设置卡片；上传需求文档后自动完成需求映射，点击“生成大纲”后切换到独立大纲编辑视图，AI 生成用例默认按“步骤内容 -> ... -> 业务动作节点 -> 预期结果内容”的横向单链节点展示，节点文本不再额外拼接“用例概述”“用例步骤N”“预期结果”等前缀，预期结果标签只挂在末尾子节点上，且“用例描述：验证XXX功能”中的 XXX 取预期结果节点的前一个节点文本；并复用系统功能全景图编辑页的画布、节点操作、用例类型、用例等级、预期结果标签、右键菜单、撤销重做、下载和页面全屏能力；同级节点已存在“预期结果”时不能再把其他同级节点设置为“预期结果”，保存时也会阻断预期结果父节点重复的大纲；点击“保存大纲并返回”后回到案例生成流程并启用最终测试用例预览；生成大纲时在当前步骤卡片内展示带阶段节点的粒子加载动画，预览区可查看“需求映射”详情并手动保存案例，页面下方展示已保存案例列表；推理强度默认“均衡”，切换到“深度 / 快速”时会额外调整内网大模型请求参数 |
...
| AI 辅助工具 | 文档核对 | - | 路由 `/ai-tools/pdf-check`；选择项目、通过日期控件选择测试版本并保存为 `YYYYMMDD`、选择同项目 PDF 模板、上传待核对 PDF 后生成系统判定，详情页支持文字级差异高亮、OCR 文本修正后重新比对，以及人工修改最终结果 |
| AI 辅助工具 | 数据生成 | - | 占位入口 |
...
| 配置管理 | PDF 模板 | - | 路由 `/config-management/pdf-templates`；按项目隔离维护文档核对模板，支持上传、下载和逻辑删除 |
...
- 末级节点标记“预期结果”后，会自动补充“用例描述：验证XXX功能”标签，其中 `XXX` 为该预期结果节点的前一个节点文本；如果没有可用父级动作节点，则回退为该末级节点文本；同级节点已存在“预期结果”时，不能再把其他同级节点设置为“预期结果”
- 保存大纲时会校验至少存在一条带“反向”标签的分支，并校验每个分支的最后一个节点都带“预期结果”标签；校验不通过不会阻断普通大纲保存，手动保存会给出提示，缺少预期结果的末级节点会在画布中标红；若多个预期结果节点会生成重复用例描述，也就是预期结果的父节点重复，则会阻断保存并提示需要先调整
...
- `/config-management/pdf-templates` 用于按项目维护文档核对模板
- `/ai-tools/pdf-check` 用于上传测试版本 PDF 并与项目模板进行文字级比对，测试版本通过日期控件选择并保存为 `YYYYMMDD`
...
- “拉取表、字段信息”会直接刷新当前数据库的表元数据，并在表格空态区分未选库、拉取中、拉取失败和数据库未返回表；成功时显示已拉取表数量
- 分步弹窗切换步骤时会保留并继续监听已选择的数据库和扫描表，避免进入后续步骤后表字段请求被禁用
- 规则类型包括：
...
- 点击“详情”后，通过弹窗展示完整提示词
- 支持新增、编辑、删除提示词；新增与编辑时必须选择所属模块
- 所属模块下拉枚举为：`通用`、`案例生成`、`需求分析`、`案例分析`、`接口自动化`、`AI助手`
...
- 各业务页面选择提示词时，只展示所属模块为 `通用` 或当前业务模块的提示词；例如案例生成页只展示 `通用` 和 `案例生成` 模块提示词
...
- 提示词来源于配置管理 > 提示词管理，仅展示 `通用` 与 `案例生成` 模块提示词；页面会优先预选 `案例生成` 模块专属提示词，其次预选 `requirement` 即“需求分析师”；默认提示词只会预填，不会跳过步骤，用户仍需点击“下一步”确认后才进入上传需求文档
...
- 未复用模板时，大纲根节点为用例名，下面按多个节点展示 AI 生成的测试用例；每条用例默认为“步骤内容 -> ... -> 业务动作节点 -> 预期结果内容”的横向单链结构，节点文本不再额外拼接“用例概述”“用例步骤N”“预期结果”等前缀，末端预期结果节点自动带“预期结果”标签，用例描述标签按预期结果节点的前一个业务动作节点生成；复用模板时，大纲根节点为用例名，二级节点为“复用模板”和“AI生成用例”，前者展示选中系统功能全景图大纲，后者展示同样单链结构的 AI 生成测试用例
- 大纲不再在案例生成步骤内嵌可拖拽画布；点击“生成大纲”后会切换到独立大纲编辑视图，直接复用系统功能全景图编辑页的思维导图画布能力，包括双击编辑节点、节点操作（新增子节点、新增同级节点、删除节点）、用例类型、用例等级、预期结果标签、右键快捷编辑、撤销、重做、下载大纲和页面全屏；保存前会阻断重复用例描述，且点击“保存大纲并返回”后，最后一步“生成测试用例”才会启用，并按已保存大纲生成最终表格预览
...
- 页面下方内嵌“测试案例记录”区块，列表字段包含生成时间、需求文档名称、操作人、案例条数，操作栏提供短按钮“预览”“大纲预览”和“导出”，避免按钮内容挤出列表区域；“大纲预览”打开只读思维导图页面，页面全屏时会把预览工具栏和画布作为同一个固定工作区展示，并临时收起左侧导航栏
...
- 配置管理中所属模块为 `通用` 或 `AI助手` 的提示词会作为可切换的回答风格来源；存在提示词时，可在页面底部切换使用
...
- `module`：可选；传入后返回所属模块为 `通用` 或该模块的提示词；未传时返回全部提示词

请求字段：

- `name`
- `module`：可选，默认 `通用`；可选值为 `通用`、`案例生成`、`需求分析`、`案例分析`、`接口自动化`、`AI助手`
- `prompt`
...
- 文档核对接口使用 `multipart/form-data`，必填 `project_id`、`test_version`、`template_id`、`pdf_file`；前端将测试版本日期保存为 `YYYYMMDD`。
...
- 变量差异忽略沿用第一版规则：手动框选变量区域、自定义正则、差异附近命中的字段关键字，以及明确的日期、证件号、手机号、金额等内置格式会被计入变量差异；中文地址不放入默认字段关键字，改由内置地址正则识别，例如 `XX省XX市XX区XXXXX`；姓名字段通过 `姓名：XXXX` 这种内置字段正则识别。
- 差异按文字级标记为缺失、新增或变更，详情页使用同一个滚动区域并排预览模板 PDF 与核对 PDF；有页面渲染图时尽量保留 PDF 源文件样式并叠加差异高亮，保留原始文件内容的旧记录缺少渲染图时会在首次详情预览时补齐并缓存到记录，后续打开不再重复渲染；核对记录创建时间由后端写入当前系统本地时间。
...
- 回归验证流程中的表清单刷新直接调用 `tables?refresh=true`，页面会展示拉取到的表数量，避免把接口失败或未选库误判为“暂无数据”
